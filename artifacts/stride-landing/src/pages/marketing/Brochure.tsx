...
const FEATURES = [
  { icon: "📷", title: "QR Attendance", desc: "Instant check-in for every student with a simple QR scan. No paper, no friction.", color: "#FBBF24" },
  { icon: "📍", title: "Smart Pick-Up", desc: "BLE proximity and parent authorization ensure every child leaves with the right person.", color: "#60A5FA" },
  { icon: "🚨", title: "Emergency Pulse", desc: "Broadcast critical alerts to all parents in seconds. Full acknowledgment tracking.", color: "#F87171" },
  { icon: "💳", title: "Online Payments", desc: "Course fees, event tickets, and marketplace items — all handled via Stripe.", color: "#4ADE80" },
...
const PLANS = [
  { name: "Core",    price: "€49",  period: "/mo", features: ["Up to 100 members", "QR Attendance", "Smart Pick-Up", "Online Payments", "SOS Emergency"], highlight: false },
  { name: "Plus",    price: "€99",  period: "/mo", features: ["Up to 300 members", "Everything in Core", "AI Roster Optimizer", "Marketplace", "Multi-association"], highlight: true },
  { name: "Premium", price: "€199", period: "/mo", features: ["Unlimited members", "Everything in Plus", "White-label branding", "Dedicated support", "Custom integrations"], highlight: false },
];

export default function Brochure() {
  return (
    <div style={{ width: "100%", background: "#0a0f1e", display: "flex", justifyContent: "center", padding: "40px 0" }}>
      <div style={{ fontFamily: "Montserrat, sans-serif", width: 1200, background: WHITE, color: "#0F172A" }}>
        ...
        <div style={{ background: NAVY, padding: "80px 100px 100px", position: "relative", overflow: "hidden", minHeight: 600 }}>
          ...
          <div style={{ fontSize: 14, fontWeight: 700, color: GOLD, letterSpacing: 3, marginBottom: 24 }}>DANCE SCHOOL MANAGEMENT PLATFORM</div>
          <div style={{ fontSize: 72, fontWeight: 900, color: WHITE, lineHeight: 1.05, letterSpacing: -2, marginBottom: 32, maxWidth: 800 }}>
            The platform your dance school deserves.
          </div>
          <div style={{ fontSize: 22, color: "rgba(255,255,255,0.65)", fontWeight: 400, lineHeight: 1.6, maxWidth: 680, marginBottom: 56 }}>
            Stride brings together attendance, payments, communications, analytics, and compliance — in a single app built specifically for dance schools.
          </div>
          ...
          <div style={{ display: "flex", gap: 16, marginTop: 60 }}>
            {["👨‍👩‍👧 For Parents", "🎓 For Operators", "⚙️ For Admins"].map(b => (
        ...
        <div style={{ padding: "80px 100px", background: BG, borderBottom: `1px solid ${BORDER}` }}>
          ...
          <div style={{ fontSize: 48, fontWeight: 900, color: NAVY, letterSpacing: -1, marginBottom: 32, lineHeight: 1.1 }}>Managing a dance school is still painfully manual.</div>
          <div style={{ display: "flex", gap: 24 }}>
            {[
              { emoji: "📱", label: "WhatsApp chaos", desc: "Parents texting at all hours. No record, no accountability." },
        ...
        <div style={{ padding: "80px 100px", background: WHITE }}>
          ...
          <div style={{ fontSize: 48, fontWeight: 900, color: NAVY, letterSpacing: -1, marginBottom: 48, lineHeight: 1.1 }}>Everything your school needs.<br />Nothing it doesn't.</div>
        ...
        <div style={{ padding: "80px 100px", background: NAVY }}>
          ...
          <div style={{ fontSize: 48, fontWeight: 900, color: WHITE, letterSpacing: -1, marginBottom: 48, lineHeight: 1.1 }}>Built for every person<br />in your school.</div>
          <div style={{ display: "flex", gap: 24 }}>
            {[
              { role: "Parents", icon: "👨‍👩‍👧", color: GOLD, items: ["Live attendance updates", "Smart Pick-Up alerts", "Event tickets & booking", "Wallet & payments", "Digital document signing"] },
              { role: "Operators", icon: "🎓", color: "#60A5FA", items: ["QR attendance scanner", "Daily roll call roster", "Substitute management", "Payroll & schedules", "Emergency SOS button"] },
              { role: "Admins", icon: "⚙️", color: "#4ADE80", items: ["Revenue dashboard", "Member management", "Course analytics", "White-label branding", "Legal compliance"] },
            ].map(r => (
          ...
          <div style={{ display: "flex", gap: 24 }}>
            {PLANS.map(p => (
              <div key={p.name} style={{ flex: 1, background: p.highlight ? NAVY : WHITE, border: p.highlight ? `2px solid ${GOLD}` : `1px solid ${BORDER}`, borderRadius: 24, padding: "36px 32px", boxShadow: p.highlight ? `0 12px 40px rgba(30,58,138,0.25)` : "0 1px 6px rgba(0,0,0,0.06)", position: "relative" }}>
                {p.highlight && <div style={{ position: "absolute", top: -14, left: "50%", transform: "translateX(-50%)", background: GOLD, borderRadius: 20, padding: "6px 20px", fontSize: 13, fontWeight: 700, color: NAVY }}>MOST POPULAR</div>}
                <div style={{ fontSize: 22, fontWeight: 800, color: p.highlight ? GOLD : NAVY, marginBottom: 8 }}>{p.name}</div>
                <div style={{ display: "flex", alignItems: "baseline", gap: 4, marginBottom: 28 }}>
                  <span style={{ fontSize: 52, fontWeight: 900, color: p.highlight ? WHITE : NAVY }}>{p.price}</span>
                  <span style={{ fontSize: 18, color: p.highlight ? "rgba(255,255,255,0.5)" : MUTED }}>{p.period}</span>
                </div>
                {p.features.map(f => (
                  <div key={f} style={{ display: "flex", gap: 10, alignItems: "center", marginBottom: 12 }}>
                    <span style={{ color: p.highlight ? GOLD : "#22C55E", fontSize: 16 }}>checkmark</span>
        ...
        <div style={{ background: NAVY, padding: "80px 100px", textAlign: "center", position: "relative", overflow: "hidden" }}>
          ...
          <div style={{ fontSize: 56, fontWeight: 900, color: WHITE, letterSpacing: -1.5, lineHeight: 1.05, marginBottom: 24 }}>
            Ready to transform<br />your dance school?
          </div>
