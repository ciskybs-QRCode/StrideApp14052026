...
function generateReceiptHtml(receipt: Receipt, sessionId: string): string {
  ...
  return `<!DOCTYPE html>
<html><head><meta charset="utf-8"/><title>Stride Receipt ${invNum}</title>
<style>*{box-sizing:border-box;margin:0;padding:0}body{font-family:Arial,sans-serif;padding:40px;color:#1E3A8A;max-width:680px;margin:auto}
.header{border-bottom:3px solid #D4AF37;padding-bottom:16px;margin-bottom:24px;display:flex;justify-content:space-between;align-items:flex-end}
...
table{width:100%;border-collapse:collapse}th{background:#1E3A8A;color:#fff;padding:10px 12px;text-align:left;font-size:12px;font-weight:700}
.tot{margin-top:20px;text-align:right;font-size:18px;font-weight:800;color:#1E3A8A}
.gold{color:#D4AF37}
.foot{margin-top:40px;text-align:center;font-size:11px;color:#9CA3AF;border-top:1px solid #E5E7EB;padding-top:16px}
@media print{body{padding:20px}}</style></head>
<body>
<div class="header">
  <div><div class="org">Stride Platform</div><div class="sub">Official Payment Receipt</div></div>
  <svg height="36" width="36" viewBox="0 0 36 36" fill="none"><rect width="36" height="36" rx="9" fill="#1E3A8A"/><path d="M9 18h18M18 10l8 8-8 8" stroke="#D4AF37" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round"/></svg>
</div>
<div class="meta">
...
${rows ? `<table><thead><tr><th>Course / Activity</th><th>Participant</th><th>Package</th><th style="text-align:right">Amount</th></tr></thead><tbody>${rows}</tbody></table>` : ""}
<div class="tot">Total Paid: <span class="gold">${sym}${total}</span></div>
<div class="foot">Stride Platform &mdash; Secure payments powered by Stripe &mdash; This is an automatically generated receipt</div>
</body></html>`;
}
...
export default function PaymentSuccess() {
  ...
  useEffect(() => {
    ...
    poll();
    // Poll up to 5 times (15 s) while pending — webhook may not have arrived yet
                  ...
                  <div className="flex items-start justify-between gap-4">
                    ...
                    {total && (
                      <div className="text-right">
                        ...
                        <p className="text-[#D4AF37] font-black text-2xl">{sym}{total}</p>
                      </div>
                    )}
                  </div>
                </div>
                    ...
                    {receipt.items.map((item, i) => (
                      <div key={i} className="px-6 py-4 flex items-center justify-between gap-4">
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-blue-300 text-xs mt-0.5">{item.participantName} · {item.packageType === "fixedBlock" ? "Full Package" : "Single Lesson"}</p>
                        </div>
