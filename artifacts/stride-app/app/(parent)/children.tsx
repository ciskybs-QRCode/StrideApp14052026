...
export default function ChildrenScreen() {
  ...
  // ── Absence scheduling state ────────────────────────────────────────────────
  const [showAbsenceModal, setShowAbsenceModal] = useState(false);
  const [absenceTab, setAbsenceTab] = useState<0 | 1>(0);
  const [futureAbsenceMode, setFutureAbsenceMode] = useState<"single" | "range">("single");
  ...
  const [absSubmitSuccess, setAbsSubmitSuccess] = useState(false);
  ...
  function buildAbsDate(d: string, m: string, y: string): string | null {
  ...
  const handleSubmitAbsence = async () => {
    ...
    if (absenceTab === 0) {
      ...
      try {
        ...
        list.unshift({
          studentId: child.id, studentName: child.name, mode: "single",
          absence_date: todayStr, note: absNote.trim(), status: "reported_today",
          created_at: new Date().toISOString(),
        });
        await AsyncStorage.setItem("stride_student_absences", JSON.stringify(list));
        api.reportStudentFutureAbsence({
          student_id: child.id, student_name: child.name,
          mode: "single", absence_date: todayStr, note: absNote.trim() || undefined,
        }).catch(() => {});
        Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);
        setAbsSubmitSuccess(true);
        setAbsNote("");
        setTimeout(() => { setAbsSubmitSuccess(false); setShowAbsenceModal(false); }, 2500);
      } catch {
        Alert.alert("Error", "Could not save the absence.");
      } finally {
        setAbsSubmitting(false);
      }
    } else {
      const dateStr = buildAbsDate(absDay, absMonth, absYear);
      if (!dateStr) { Alert.alert("Invalid Date", "Please enter a valid future date (DD/MM/YYYY)."); return; }
      const endDateRaw = futureAbsenceMode === "range"
        ? buildAbsDate(absEndDay, absEndMonth, absEndYear)
        : null;
      if (futureAbsenceMode === "range" && !endDateRaw) {
      ...
      try {
        ...
        list.unshift({
          ...
          mode: futureAbsenceMode, absence_date: dateStr,
          end_date: endDateRaw ?? undefined, note: absNote.trim(),
          status: "scheduled_future", created_at: new Date().toISOString(),
        });
        await AsyncStorage.setItem("stride_student_absences", JSON.stringify(list));
        api.reportStudentFutureAbsence({
          student_id: child.id, student_name: child.name,
          mode: futureAbsenceMode, absence_date: dateStr,
          end_date: endDateRaw ?? undefined, note: absNote.trim() || undefined,
        }).catch(() => {});
        Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);
        setAbsSubmitSuccess(true);
        resetAbsenceForm();
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      {/* Absence Schedule Modal */}
      <Modal visible={showAbsenceModal} transparent animationType="slide" onRequestClose={() => setShowAbsenceModal(false)}>
        <View style={styles.modalOverlay}>
          <View style={[styles.modalCard, { position: "relative", maxHeight: "90%", paddingTop: 44 }]}>
            <Pressable style={{ position: "absolute", top: 12, right: 14, zIndex: 20, padding: 4 }} onPress={() => setShowAbsenceModal(false)} hitSlop={14}>
              <Ionicons name="close-circle" size={30} color="#9CA3AF" />
            </Pressable>
            <ScrollView showsVerticalScrollIndicator={false} style={{ width: "100%" }} contentContainerStyle={{ paddingBottom: 4 }}>
              <Text style={[styles.modalTitle, { color: colors.primary }]}>Absence Schedule</Text>
              {child && (
                <View style={styles.absChildRow}>
                  <View style={[styles.absChildAvatar, { backgroundColor: colors.primary }]}>
                    <Text style={styles.absChildAvatarText}>{child.name.charAt(0)}</Text>
                  </View>
                  <Text style={[styles.absChildName, { color: colors.primary }]}>{child.name}</Text>
                </View>
              )}

              {/* Segmented control */}
              <View style={[styles.absSegControl, { backgroundColor: colors.muted }]}>
                {(["Report Today", "Schedule Future"] as const).map((label, idx) => (
                  <Pressable
                    key={label}
                    style={[styles.absSegTab, absenceTab === idx && { backgroundColor: colors.primary }]}
                    onPress={() => {
                  ...
                  >
                    <Ionicons
                      name={idx === 0 ? "today" : "calendar"}
                      size={13}
                      color={absenceTab === idx ? "#FFF" : colors.mutedForeground}
                    />
                    <Text style={[styles.absSegTabText, { color: absenceTab === idx ? "#FFF" : colors.mutedForeground }]}>
                      {label}
                    </Text>
                  </Pressable>
                ))}
              ...
              {absSubmitSuccess ? (
                <View style={{ alignItems: "center", paddingVertical: 28, gap: 12 }}>
                  ...
                  <Text style={{ fontSize: 16, fontWeight: "800", color: "#10B981" }}>
                    {absenceTab === 0 ? "Absence Reported" : "Absence Scheduled"}
                  </Text>
                  <Text style={{ fontSize: 13, color: colors.mutedForeground, textAlign: "center", lineHeight: 18 }}>
                    {absenceTab === 0
                      ? "Today's absence has been recorded. The kiosk will flag this student as excused."
                      : "Future absence saved. On the target date the kiosk will flag this student as excused absence."}
                  </Text>
                </View>
              ) : (
                <>
                  {absenceTab === 0 ? (
                    <>
                      <View style={[styles.absTodayBanner, { backgroundColor: "#EEF2FF" }]}>
                        <Ionicons name="today" size={16} color={colors.primary} />
                        <Text style={[styles.absTodayText, { color: colors.primary }]}>
                          Reporting absence for today — {new Date().toLocaleDateString("en-GB", { weekday: "short", day: "numeric", month: "short" })}
                        </Text>
                      </View>
                      <Text style={[styles.modalLabel, { color: colors.primary, marginTop: 12 }]}>Reason (Optional)</Text>
                      <TextInput
                        style={[styles.modalInput, styles.absNoteInput, { borderColor: colors.border }]}
                        value={absNote}
                        onChangeText={setAbsNote}
                        placeholder="e.g. Illness, Medical appointment..."
                        placeholderTextColor={colors.mutedForeground}
                        multiline
                        numberOfLines={3}
                      />
                    ...
                    <>
                      ...
                      <Text style={[styles.modalLabel, { color: colors.primary, marginTop: 12 }]}>Absence Type</Text>
                      <View style={styles.absModeRow}>
                        {([
                          ...
                          <Pressable
                            ...
                            style={[
                              ...
                              futureAbsenceMode === opt.key && { backgroundColor: colors.primary },
                            ]}
                            onPress={() => setFutureAbsenceMode(opt.key)}
                          >
                            <Text style={[styles.absModePillText, { color: futureAbsenceMode === opt.key ? "#FFF" : colors.primary }]}>
                              {opt.label}
                            </Text>
                          </Pressable>
                        ))}
                      ...
                      <Text style={[styles.modalLabel, { color: colors.primary }]}>
                        {futureAbsenceMode === "range" ? "Start Date" : "Absence Date"}
                      </Text>
                      <View style={styles.absDateRow}>
                        <TextInput
                          style={[styles.absDateCell, { borderColor: colors.border, color: colors.foreground }]}
                        ...
                        <Text style={[styles.absDateSep, { color: colors.mutedForeground }]}>/</Text>
                        <TextInput
                          style={[styles.absDateCellWide, { borderColor: colors.border, color: colors.foreground }]}
                      ...
                      {futureAbsenceMode === "range" && (
                        <>
                          <Text style={[styles.modalLabel, { color: colors.primary }]}>End Date</Text>
                          <View style={styles.absDateRow}>
                            <TextInput
                              style={[styles.absDateCell, { borderColor: colors.border, color: colors.foreground }]}
                      ...
                      <Text style={[styles.modalLabel, { color: colors.primary }]}>Note (Optional)</Text>
                      <TextInput
                        style={[styles.modalInput, styles.absNoteInput, { borderColor: colors.border }]}
                        value={absNote}
                        onChangeText={setAbsNote}
                  ...
                  {/* Kiosk info note */}
                  <View style={[styles.absKioskNote, { backgroundColor: "#F0FDF4", borderColor: "#BBF7D0" }]}>
                    <Ionicons name="shield-checkmark-outline" size={14} color="#10B981" />
                    <Text style={styles.absKioskNoteText}>
                      On the target date, the kiosk will automatically flag this student as{" "}
                      <Text style={{ fontWeight: "700" }}>Excused Absence</Text>, preventing any missing-child alerts.
                    </Text>
                  </View>
                  ...
                  <View style={{ flexDirection: "row", gap: 12, marginTop: 20 }}>
                    <Pressable
                      ...
                      onPress={() => { setShowAbsenceModal(false); resetAbsenceForm(); }}
                    >
                      <Text style={[styles.modalBtnText, { color: colors.primary }]}>Cancel</Text>
                    </Pressable>
                    <Pressable
                      style={[styles.modalBtn, { backgroundColor: "#FBBF24", flex: 1, opacity: absSubmitting ? 0.6 : 1 }]}
                      onPress={handleSubmitAbsence}
                      disabled={absSubmitting}
                    >
                      <Text style={[styles.modalBtnText, { color: "#1E3A8A" }]}>
                        {absSubmitting ? "Saving..." : absenceTab === 0 ? "Report Now" : "Schedule"}
                      </Text>
                    </Pressable>
                  </View>
                </>
...
const styles = StyleSheet.create({
  ...
  // ── Absence Schedule Modal ────────────────────────────────────────────────
  absChildRow: { flexDirection: "row", alignItems: "center", gap: 10, marginBottom: 16, backgroundColor: "#EEF2FF", borderRadius: 12, padding: 12 },
  absChildAvatar: { width: 36, height: 36, borderRadius: 18, alignItems: "center", justifyContent: "center" },
  absChildAvatarText: { color: "#FFF", fontWeight: "700", fontSize: 15 },
  absChildName: { fontSize: 15, fontWeight: "700" },
  absSegControl: { flexDirection: "row", borderRadius: 12, padding: 3, marginBottom: 16 },
  absSegTab: { flex: 1, borderRadius: 9, paddingVertical: 9, alignItems: "center", flexDirection: "row", justifyContent: "center", gap: 6 },
  absSegTabText: { fontSize: 13, fontWeight: "700" },
  absTodayBanner: { flexDirection: "row", alignItems: "center", gap: 8, borderRadius: 10, padding: 12, marginBottom: 4 },
  absTodayText: { fontSize: 13, fontWeight: "600", flex: 1 },
  absNoteInput: { minHeight: 72, textAlignVertical: "top" },
  absModeRow: { flexDirection: "row", gap: 8, marginBottom: 4 },
  absModePill: { flex: 1, borderRadius: 10, paddingVertical: 10, alignItems: "center", borderWidth: 1.5 },
  absModePillText: { fontSize: 13, fontWeight: "700" },
  absDateRow: { flexDirection: "row", alignItems: "center", gap: 6, marginBottom: 4 },
  absDateCell: { width: 50, borderWidth: 1.5, borderRadius: 10, paddingVertical: 11, textAlign: "center", fontSize: 15, fontWeight: "600" },
  absDateSep: { fontSize: 16 },
  absDateCellWide: { flex: 1, borderWidth: 1.5, borderRadius: 10, paddingVertical: 11, textAlign: "center", fontSize: 15, fontWeight: "600" },
  absKioskNote: { flexDirection: "row", alignItems: "flex-start", gap: 8, borderRadius: 10, padding: 12, borderWidth: 1, marginTop: 12 },
  absKioskNoteText: { flex: 1, fontSize: 12, color: "#047857", lineHeight: 17 },
});
