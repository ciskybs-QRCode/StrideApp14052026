...
const PROFILE_KEY = "stride_profile_extra_v1";
...
const EMPTY: ProfileExtra = { firstName: "", lastName: "", gender: "", phone: "", address: "", taxId: "" };

const GENDER_OPTIONS = ["Male", "Female", "Non-binary", "Prefer not to say"];

function taxIdLabel(): string {
...
export default function OperatorProfileEditScreen() {
  ...
  const [form, setForm] = useState<ProfileExtra>(EMPTY);
  ...
  const field = (key: keyof ProfileExtra) => (v: string) => setForm(p => ({ ...p, [key]: v }));
  ...
  const TAX_LABEL = taxIdLabel();

  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      <ScreenHeader title="Information Settings" onBack={() => router.navigate("/(operator)/settings" as never)} />
      <ScrollView
      ...
      >
            ...
            {GENDER_OPTIONS.map(opt => {
              ...
              return (
                <Pressable
                  ...
                  onPress={() => { Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light); setForm(p => ({ ...p, gender: opt })); }}
                >
                  <Text style={[styles.genderPillText, { color: active ? "#FFF" : colors.mutedForeground }]}>{opt}</Text>
                </Pressable>
              );
            })}
          </View>
        ...
        <View style={[styles.card, { backgroundColor: colors.card }]}>
          ...
          <View style={[styles.fieldWrap, styles.fieldDivider, { borderTopColor: colors.border }]}>
            ...
            <TextInput
              style={[styles.fieldInput, styles.fieldInputMulti, { borderColor: colors.border, backgroundColor: colors.muted, color: colors.foreground }]}
              value={form.address}
        ...
        <View style={[styles.card, { backgroundColor: colors.card }]}>
          <View style={styles.fieldWrap}>
            <Text style={[styles.fieldLabel, { color: colors.mutedForeground }]}>{TAX_LABEL}</Text>
            <TextInput
              style={[styles.fieldInput, { borderColor: colors.border, backgroundColor: colors.muted, color: colors.foreground }]}
              value={form.taxId}
        ...
        {/* ── Save ── */}
        <Pressable
          style={[styles.saveBtn, { backgroundColor: saved ? "#10B981" : colors.primary, opacity: saving ? 0.7 : 1 }]}
          onPress={handleSave}
          disabled={saving}
        >
          <Ionicons name={saved ? "checkmark-circle" : "save-outline"} size={20} color="#FBBF24" />
...
const styles = StyleSheet.create({
  ...
  sectionLabel: { fontSize: 13, fontWeight: "800", textTransform: "uppercase", letterSpacing: 0.8, marginBottom: 8, marginTop: 8 },
  card: {
  ...
  fieldInput: { borderRadius: 10, paddingHorizontal: 12, paddingVertical: 10, fontSize: 14, borderWidth: 1 },
  ...
  saveBtn: {
    ...
    borderRadius: 18, paddingVertical: 16, marginTop: 8, marginBottom: 16,
  },
